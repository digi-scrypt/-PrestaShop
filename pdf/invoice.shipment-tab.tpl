 ...
 *}
{foreach $products_by_shipment as $idx => $shipment name="shipments_loop"}
	{if !$smarty.foreach.shipments_loop.first}
		<tr>
	...
	<table width="100%" cellpadding="4" cellspacing="0">
		<tr>
			<td style="font-weight: bold; font-size: 9pt;">
				{if $idx eq 'virtual_products'}
					{l s='Virtual product(s)' d='Shop.Pdf' pdf='true'}
				{else}
					{$smarty.foreach.shipments_loop.iteration}/{$smarty.foreach.shipments_loop.total} - {l s='Shipped by' d='Shop.Pdf' pdf='true'} {$shipment.carrierName}
					{if $shipment.trackingNumber}
						({l s='Tracking number' d='Shop.Pdf' pdf='true'}: {$shipment.trackingNumber})
					{/if}
				{/if}
			</td>
		</tr>
